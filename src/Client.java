...
// Testing the client updater before release
/**
 * Handles the main client class
 * 
 * @author Sythescape Team
 */
public class Client extends ClientEngine {
	...
	private int getChatColor() {
		...
		if (chatColorHex.equals("") || chatColorHex == null) chatColorHex = "00FFFF";
	...
	public Socket openSocket(int port) throws IOException {
		return new Socket(InetAddress.getByName(server), 43594);
	}
	...
	private void buildInterfaceMenu(int y, RSInterface rsinterface, int mouseX, int x, int mouseY, int scrollPosition) {
		...
		if (rsinterface == null) return; // bug patched
		...
		for (int childIndex = 0; childIndex < rsinterface.children.length; childIndex++) {
			...
			if (child == null) continue; // bug patched
	...
	private void drawTabArea() {
		...
		if (showTabComponents) {
			...
			if (invOverlayInterfaceID != -1) {
				...
				//System.out.println("Drawing tab " + tabID + ", id: " + tabInterfaceIDs[tabID]);
	...
	public void resetLogout() {
		...
		loggedIn = false;
		loginScreenState = 0;
		loginMessage1 = "Welcome to Sythescape";
	...
	public static void main(String args[]) {
		...
		try {
			...
			server = args != null && args.length == 1 ? args[0] : ClientConstants.SERVER_IP;
...
/*	static {
		try {
			BACKGROUND = new Sprite(new URL("http://www.sythescape.com/Daniel/11.png"));
			LOADING_BAR = new Sprite(new URL("http://www.sythescape.com/Daniel/12.png"));
	...
	public void drawLoadingText(int percent, String message) {
		...
		//DrawingArea.drawRectangle(0,0,frameWidth, frameHeight, 0, 0);
	...
	public void login(String username, String password, boolean flag) {
		...
		try {
			...
			socketStream = new RSSocket(this, openSocket(ClientConstants.SERVER_PORT ));
			...
			if (k == 6) {
				loginMessage1 = "Sythescape has been updated!";
				loginMessage2 = "Please download the newest client.";
			//	openURL("http://www.sythescape.com/downloads/Client.jar");
				return;
			}
			if (k == 7) {
	...
	private boolean doWalkTo(int i, int j, int k, int i1, int j1, int k1, int l1, int i2, int j2, boolean flag, int k2) {
		...
		if(anIntArrayArray901.length <= j2 || anIntArrayArray901[j2].length <= j1)
			return false;
...
//test
	...
	private void drawInterface(int j, int k, RSInterface class9, int l) {
		...
		//System.out.println("Interface id: " + class9.id);

		refreshScreenOptions();
		if (class9 == null) return; // bug patched
		...
		//	System.out.println("Children id: " + class9.children[j2] + " " + child.type);
			if (child == null) continue; // bug patched
			k2 += child.anInt263;
			l2 += child.anInt265;
			if (child.contentType > 0)
...
/*			if (child.valueIndexArray != null && child.valueIndexArray[0] != null && child.valueIndexArray[0].length >= 2){
				StringBuilder sb = new StringBuilder("Child: ");
				for (int jj = 0; jj < child.valueIndexArray.length; jj++) {
				sb.append("[" + jj + "] = (");
				for (int i = 0; i < child.valueIndexArray[jj].length; i++) {
					sb.append(child.valueIndexArray[jj][i]);
					if (i != child.valueIndexArray[jj].length - 1) sb.append(", ");
				}
					sb.append("), ");
				}
	...
	private void draw3dScreen() {
		...
		if (clientData) {
			...
			smallText.method385(textColour, "Coords: " + x + ", " + y + " base: " + baseX + ", " + baseY + " mpos: " + myPlayer.x + ", " + myPlayer.y, frameHeight - 201, 5);
	...
	private void drawCounterOnScreen() {
		...
		smallText.method382(0xffffff, x - 38 - i - digits - 12, "Total:", y + 30, true);
		if (xpCounter >= 0) {
			smallText.method382(0xffffff, x + 1 - i, "+" + NumberFormat.getIntegerInstance().format(xpCounter), y + 30, true);
		}
		int currentIndex = 0;
		int offsetY = 0;
		int stop = 40;
